...
export const CANVAS_SKILLS: CanvasSkill[] = [
  ...
  },
  {
    ...
    body: [
      ...
      '   mode, once you and the human agree what to build, `create_sprint` (its',
      '   outcome + which vision gap it closes).',
  ...
  },
  {
    ...
    body: [
      ...
      '5. When the work is complete, `update_issue_status` → `in_review` (now under audit).',
      '6. **Self-audit** (mandatory): spawn adversarial subagents to *refute* your work',
      '   against the `verify` criteria and the vision — assume it is wrong, hunt for what',
      '   breaks. Fix everything real they surface, and re-audit until it survives.',
      '7. **`update_issue_status` → `done` once the self-audit passes — this is REQUIRED;',
      '   `in_review` is not the finish line.** Then `comment_issue` with a one-line',
      '   summary of what you did and what the audit checked. An issue only counts as',
      "   finished — and only unblocks its dependents — when it's `done`.",
