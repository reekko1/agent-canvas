...
const api: CanvasApi = {
  ...
  loadIssueStore: () => ipcRenderer.invoke('load-issue-store'),
  issueAction: (action) => ipcRenderer.invoke('issue-action', action),
  onIssueUpdate: (cb) => subscribe('issue-update', cb),
  loadMastermindSkills: () => ipcRenderer.invoke('load-mastermind-skills'),
  onSkillsUpdate: (cb) => subscribe('skills-update', cb),
  ...
  notifyVoicePlaying: (playing) => ipcRenderer.send('voice-playing', playing),
