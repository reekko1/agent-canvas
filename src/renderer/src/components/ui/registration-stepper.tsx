      ...
      <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
        <div className="flex flex-col space-y-1.5 p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-status-done bg-status-done/10 px-2 py-1 rounded-full">
              {headerStatus}
            </span>
          </div>
        </div>
                ...
                <li key={step.title} className="overflow-hidden">
                  <div className="flex items-start gap-4">
                    <div className="flex flex-col items-center">
                            ...
                            >
                              ...
                              <span
                                className={cn(
                                  ...
                                  isActive &&
                                    "text-primary-foreground bg-primary rounded-full h-6 w-6 m-1",
                                )}
                              >
                                {stepNumber}
                              </span>
                      ...
                      {/* Connector line — fades out while its step is active:
                          the expanded content owns that space, and a line
                          dangling under the highlighted circle reads as
                          pointing at nothing. */}
                      {index < steps.length - 1 && (
                        <motion.div
                          animate={{ opacity: isActive ? 0 : 1 }}
                          transition={{ duration: 0.3 }}
                          className="mt-2 h-8 w-px bg-border"
                        />
                      )}
                    </div>
