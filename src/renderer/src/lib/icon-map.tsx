...
// Param typed ComponentType<any>: third-party icon prop types drift across
// versions (size/stroke as string|number, wider weight unions); the adapter
// body passes concrete, correct props, so we don't re-type each library here.
function tabler(Icon: ComponentType<any>): IconComponent {
...
function phosphor(Icon: ComponentType<any>): IconComponent {
...
// Lucide ships `size`/`strokeWidth` as `string | number`, which doesn't satisfy
// IconComponent's narrower props under strict TS (propTypes mismatch). These are
// used through IconComponent and assign cleanly via ComponentType<any>; the other
// libraries go through adapters, so this map is the only raw assignment.
const lucideMap: Record<IconName, ComponentType<any>> = {
...
// ComponentType<any> because a few entries fall back to raw lucide components
// (hugeicons has no play/pause), which don't satisfy IconComponent under strict
// TS — same reason as lucideMap.
const hugeiconsMap: Record<IconName, ComponentType<any>> = {
